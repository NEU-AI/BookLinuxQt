...
void QWidgetSerialTx::sendEcgPkg()
{
    ...
    // 取ECG2数据
    if(ecgDataIndex > (int)sizeof(ecgData))
        ecgDataIndex = 0;
    ...
    for(i=0; i < 3; i++)
    {
        dataBuf[1] = dataBuf[1] | (ecg[i]&0x80 >> i*2);
        dataBuf[1] = dataBuf[1] | ((ecg[i]&0x8000) >> (i*2+8));
        dataBuf[i*2+2] = 0x80 | (ecg[i] & 0xFF);
        dataBuf[i*2+3] = 0x80 | ((ecg[i]>>8) & 0xFF);
    }
    dataBuf[8] = 0x80; // 状态位
    dataBuf[9] = 0x80; // 校验位
